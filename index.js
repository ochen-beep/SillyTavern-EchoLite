 ...
 * Removed: Livestream mode, most built-in styles, extra legacy surface
 * Added: Mobile ST input fix, 100dvh, i18n (ru-ru), touch targets, post/swipe commentary cache
...
let currentPostId = null; // msgId (chat[] index as String) of the post shown in the bar
 ...
 // i18n
    const RU = {
        'EchoLite': 'EchoLite',
        'EchoLite Description': 'EchoLite — отдельная лента комментариев, которая реагирует на контекст текущей сцены.',
        'EchoLite Settings': 'Настройки EchoLite',
        ...
        'Uses your current ST credentials.': 'Использует текущие учётные данные ST.',
        '-- Select Profile --': '-- Выберите профиль --',
        'Display': 'Отображение',
        'Style': 'Стиль',
        'Position': 'Позиция',
        ...
        'Include Past EchoLite Commentary': 'Включать прошлые комментарии EchoLite',
        'Adds previous EchoLite commentary to the next prompt.': 'Добавляет прошлые комментарии EchoLite в следующий промпт.',
        'Include in Context': 'Включить в контекст',
        'Persona': 'Персона',
        'Character Description(s)': 'Персонаж',
        'World Info / Lorebook': 'World Info / Лорбук',
        'Additional prompt context for commentary generation.': 'Дополнительный контекст, который попадёт в промпт генерации.',
        ...
        'No-Save Mode': 'Режим «Без сохранения»',
        'Does not save commentary to cache. Post navigation is hidden.': 'Не сохраняет комментарии в кэш. Навигация по постам скрыта.',
        'Commentary is not saved. Post navigator is hidden.': 'Комментарии не сохраняются. Навигатор постов скрыт.',
        'Base Prompt': 'Базовый',
        'Custom': 'Настраиваемый',
        'Base RealDiscord prompt. Blocks are read-only.': 'Базовый промпт RealDiscord. Блоки доступны только для чтения.',
        'Nicknames': 'Ники',
        'Commenter Types': 'Типы комментаторов',
        'Language and Tone': 'Язык и тон',
        'Fill in the blocks you want to override. Empty blocks use the base values.': 'Заполните блоки, которые хотите переопределить. Пустые блоки используют базовые значения.',
        'Describe the nickname style. Example: English-only nicknames, minimalist, no digits...': 'Опишите стиль ников. Например: только английские ники, минималистичные, без цифр...',
        'List commenter types. Example:\n- Skeptics — doubt everything\n- Fans — lose words from excitement\n- Critics — quote and dissect every line': 'Перечислите типы пользователей. Например:\n- Скептики — сомневаются во всём\n- Фанаты — от восторга теряют слова\n- Критики — цитируют и разбирают каждую фразу',
        'Describe the language and tone. Example: English, no profanity, short sentences, Twitter-style...': 'Опишите язык и тон. Например: английский, без мата, короткие предложения, твиттер-стиль...',
        'Enable Jailbreak': 'Включить Jailbreak',
        'Adds an extra instruction block only for EchoLite commentary generation.': 'Добавляет дополнительный блок инструкций только для генерации комментариев EchoLite.',
        'Jailbreak': 'Jailbreak',
        'Jailbreak Role': 'Роль Jailbreak',
        'Jailbreak Block': 'Блок Jailbreak',
        'System': 'Система',
        'User': 'Пользователь',
        'Additional instruction for commentary generation...': 'Дополнительная инструкция для генерации комментариев...',
        'Recommended default:': 'Рекомендуется по умолчанию:',
        'Helps reduce refusals and moralizing for fictional commentary without changing your main ST prompts.': 'Помогает уменьшить отказы и морализаторство для вымышленных комментариев, не меняя основные промпты ST.',
        'API URL': 'URL API',
        'Model': 'Модель',
        'Preset': 'Пресет',
        'API Key': 'API-ключ',
        ...
        'System Font': 'Системный',
        'Mono': 'Моно',
        'Serif': 'С засечками',
        'Comic': 'Комикс',
        'Refreshes commentary automatically when new AI replies appear.': 'Автоматически обновляет комментарии, когда появляются новые ответы AI.',
        'Uses EchoLite\'s own response token limit instead of the current preset.': 'Использует собственный лимит токенов EchoLite вместо лимита текущего пресета.',
        'Adds recent visible chat messages before the current post.': 'Добавляет недавние видимые сообщения чата перед текущим постом.',
...
if (chatId !== currentChatId) {
 ...
 pendingSwipeIdx = null;
     ...
     */
    /**
     * Returns a stable fingerprint for a specific post+swipe.
     * Uses the full raw text instead of a truncated prefix so cache invalidation
     * still works when only the tail of a long message changes.
     *
     * The fingerprint is synchronous on purpose: cache restore/save paths rely on
     * it and should not become async just for hashing.
     */
    ...
    /**
     * Small stable string hash for cache fingerprints.
     * Version prefix ensures old truncated fingerprints are treated as stale and
     * self-cleaned by the existing cache validation path.
     */
    ...
    function restoreCachedCommentary(msgId, swipeIdx) {
        try {
            ...
            const cachedHtml = getCachedPost(mid, sidx);
            if (cachedHtml) {
                setDiscordText(cachedHtml);
                currentPostId   = mid;
            ...
            // Cache entry exists but fingerprint is stale -> remove silently.
            if (store.posts[mid]?.[sidx]) {
                delete store.posts[mid][sidx];
                if (Object.keys(store.posts[mid]).length === 0) delete store.posts[mid];
                if (store.current?.msgId === mid && String(store.current?.swipeIdx ?? 0) === sidx) {
    ...
    /**
     * Resolve the current swipe index for a specific chat message.
     * Returns 0 if unavailable.
     */
    function getMessageSwipeIdx(msgId) {
        try {
            const ctx = getSTContext();
            const msg = ctx.chat?.[parseInt(msgId)];
    ...
    function isCommentaryDisplayEligible(msg) {
        return !!msg && !msg.is_user && !msg.is_system;
    }

    function isCommentaryGenerationEligible(msg) {
...
Based on the scene context, generate {{count}} Discord chat messages. Use a natural mix of message types. Not every message needs reactions — only the ones that genuinely hit.`;
	...
	// ───────────────────────────────────────────────────────────
		 ...
		 */
			function hasActiveChatContext(ctx) {
				try {
					return !!ctx?.chatId;
				} catch {
					return false;
			...
			// do NOT inject persona, character card or WI/lorebook into EchoLite context.
			// In this state we only want the visible assistant greeting/history block.
			if (!hasActiveChatContext(ctx)) {
				return '';
			}
		 ...
		 */
			...
			try {
			if (!ctx) ctx = getSTContext();
			if (!hasActiveChatContext(ctx)) return null;
		...
		if (typeof ctx.getWorldInfoPrompt === 'function') {
			const configuredBudget = Number.parseInt(settings.wiTokenBudget, 10);
...
// In noSaveMode: do not save or reuse cache, but still respect the explicitly selected/current post
 // whenever it is generation-eligible. Only fall back to the last AI post when there is no valid target.
 if (settings.noSaveMode) {
     const ctx = getSTContext();
     const targetMsg = ctx.chat?.[parseInt(resolvedMsgId)];
     if (!isCommentaryGenerationEligible(targetMsg)) {
         const last = resolveLastAIPost();
         if (last) {
             resolvedMsgId    = last.msgId;
             resolvedSwipeIdx = last.swipeIdx;
 ...
 generationInProgress = true;
        ...
        try {
            ...
            let systemPrompt = resolveSTMacro(stylePrompt);
            ...
            const jailbreakBlock = settings.enableJailbreakBlock
                ? resolveSTMacro(String(settings.jailbreakText || '').trim())
                : '';
            if (jailbreakBlock) {
                if (settings.jailbreakRole === 'user') {
                    userPrompt = `${jailbreakBlock}\n\n${userPrompt}`;
                } else {
 ...
 // (контент сцены) раздельно, чтобы основной пресет ST не влиял на запрос.
    ...
    async function callSTDefaultAPI(systemPrompt, userPrompt, signal, ctx) {
        const parsedMaxTokens = Number.parseInt(settings.maxTokens, 10);
        const maxTokens = settings.overrideMaxTokens && Number.isFinite(parsedMaxTokens) && parsedMaxTokens > 0
            ? parsedMaxTokens
            : undefined;
    ...
    async function callOllamaAPI(systemPrompt, userPrompt, signal) {
        ...
        const parsedMaxTokens = Number.parseInt(settings.maxTokens, 10);
        if (settings.overrideMaxTokens && Number.isFinite(parsedMaxTokens) && parsedMaxTokens > 0) {
            body.options = { num_predict: parsedMaxTokens };
        }
    ...
    async function callConnectionProfile(systemPrompt, userPrompt, signal) {
        ...
        if (window.EchoLiteConnectionUtils) {
            return await window.EchoLiteConnectionUtils.generateWithProfile(
                ...
                (() => {
                    const parsedMaxTokens = Number.parseInt(settings.maxTokens, 10);
                    return settings.overrideMaxTokens && Number.isFinite(parsedMaxTokens) && parsedMaxTokens > 0
                        ? parsedMaxTokens
                        : undefined;
                })()
            );
 ...
 // REALDISCORD PARSER — replies + reactions
    ...
    // SETTINGS MANAGEMENT
    // ───────────────────────────────────────────────────────────
    function syncElementsFromSettings(fieldMap = SETTINGS_FIELD_MAP) {
    ...
    function loadSettings() {
        ...
        syncElementsFromSettings();

        settings.openaiUrl = normalizeOpenAIBaseUrl(settings.openaiUrl);
    ...
    function updateSourceVisibility() {
        ...
        const openAiUrlInput = document.getElementById('discord_openai_url');
        if (openAiUrlInput && src === 'openai') {
            const normalized = normalizeOpenAIBaseUrl(settings.openaiUrl);
            if (settings.openaiUrl !== normalized) settings.openaiUrl = normalized;
    ...
    function updateMaxTokensVisibility() {
        const el = document.getElementById('discord_max_tokens_container');
        if (el) el.hidden = !settings.overrideMaxTokens;
    }
    ...
    function updateJailbreakVisibility() {
        const el = document.getElementById('discord_jailbreak_block_container');
        if (el) el.hidden = !settings.enableJailbreakBlock;
    }

    function normalizeOpenAIBaseUrl(url) {
        const raw = String(url || '').trim();
        if (!raw) return defaultSettings.openaiUrl;
    ...
    // PANEL RENDERING
    // ───────────────────────────────────────────────────────────
    function getEffectivePanelPosition(pos) {
        const desired = pos || settings.position || 'bottom';
        const isMobile = window.innerWidth <= 768;
        // On mobile/Termux: force bottom position for any non-bottom layout.
        // Side panels have no room, and popout is unstable with soft keyboard / WebView viewport changes.
        return (isMobile && desired !== 'bottom') ? 'bottom' : desired;
    }

    function mountPanel($bar, pos) {
        if (pos === 'bottom') {
            ...
            jQuery('#leftSendForm, #left-nav-panel').first().after($bar);
            if (!jQuery('#leftSendForm, #left-nav-panel').length) jQuery('body').prepend($bar);
        } else if (pos === 'right' || pos === 'popout') {
    ...
    function renderPanel() {
        ...
        const pos = getEffectivePanelPosition(settings.position);
...
<div id="discordBar" class="ec_${pos}${settings.collapsed ? ' ec_collapsed' : ''}" data-style=\"${settings.style || 'realdiscord'}\">
    ...
    function updatePanelIcons() {
        ...
        const bar = document.getElementById('discordBar');
        if (bar) bar.dataset.style = settings.style || 'realdiscord';
    }
 ...
 .off('change', '#discord_user_count')
            ...
            .off('change', '#discord_font_family')
            .off('change', '#discord_auto_update')
            .off('change', '#discord_override_max_tokens')
            .off('change', '#discord_max_tokens')
 ...
 .off('change', '#discord_enable_jailbreak_block')
 .off('change', '#discord_jailbreak_role')
 .off('input change', '#discord_jailbreak_text')
 ...
 .on('click', '#ec_power_btn', function () {
        ...
        jQuery(document)
            ...
            .on('change', '#discord_openai_url', function () {
                settings.openaiUrl = normalizeOpenAIBaseUrl(this.value);
                this.value = settings.openaiUrl;
                saveSettings();
            })
            ...
            .on('change', '#discord_position', function () {
                settings.position = getEffectivePanelPosition(this.value);
                this.value = settings.position;
                saveSettings();
                removePanel();
                renderPanel();
            })
            ...
            .on('change', '#discord_font_family', function () {
                settings.fontFamily = this.value || 'system';
                applyFontFamily(settings.fontFamily);
                saveSettings();
            })
            .on('change', '#discord_auto_update', function () { settings.autoUpdate = this.checked; saveSettings(); })
            .on('change', '#discord_override_max_tokens', function () {
                settings.overrideMaxTokens = this.checked;
                saveSettings();
                updateMaxTokensVisibility();
            })
        .on('change', '#discord_max_tokens', function () {
            const parsedValue = Number.parseInt(this.value, 10);
            settings.maxTokens = Number.isFinite(parsedValue) && parsedValue > 0 ? parsedValue : '';
            if (settings.maxTokens === '') this.value = '';
            saveSettings();
        })
            ...
            .on('change', '#discord_enable_jailbreak_block', function () {
                settings.enableJailbreakBlock = this.checked;
                saveSettings();
                updateJailbreakVisibility();
            })
            .on('change', '#discord_jailbreak_role', function () {
                settings.jailbreakRole = this.value === 'user' ? 'user' : 'system';
                saveSettings();
            })
            .on('input change', '#discord_jailbreak_text', function () {
                settings.jailbreakText = this.value;
                saveSettings();
            })
    ...
    function handleManualRefresh() {
        ...
        const ctx = getSTContext();
        const msg = currentPostId !== null ? ctx.chat?.[parseInt(currentPostId)] : null;

        // Hidden/display-only current post: never generate, only try to restore cache.
        if (msg && !isCommentaryGenerationEligible(msg)) {
            restoreCachedCommentary(currentPostId, currentSwipeIdx);
            setCurrentPost(currentPostId, currentSwipeIdx);
            return;
        }

        // Preferred path: reroll the explicitly selected/current generation-eligible post.
        if (msg && isCommentaryGenerationEligible(msg)) {
            if (!settings.noSaveMode) {
                clearCachedCommentary(currentPostId);
            }
            setDiscordText('');
        ...
        // Fallback: no valid current target. Preserve old refresh behavior.
        if (!settings.noSaveMode && currentPostId !== null) {
            clearCachedCommentary(currentPostId);
        }
        setDiscordText('');
    ...
    function setOpenAIPresetUrl(preset) {
        ...
        if (urls[preset]) {
            settings.openaiUrl = normalizeOpenAIBaseUrl(urls[preset]);
            const urlInput = document.getElementById('discord_openai_url');
            if (urlInput) urlInput.value = settings.openaiUrl;
        }
    }
    ...
    // ───────────────────────────────────────────────────────────
    // EXPORT COMMENTARY AS IMAGE
    // Stable export path: build a dedicated offscreen export card,
    // render that controlled DOM with html2canvas, save PNG by default.
    // This avoids mutating the live commentary pane and is more reliable
    // on mobile/Termux/WebView than screenshotting the active scroll area.
    // ───────────────────────────────────────────────────────────
    function getExportMeta() {
        ...
        return {
            ...
            swipeIdx: currentSwipeIdx ?? 0,
        };
    }

    function ensureExportRoot() {
        let root = document.getElementById('ec_export_root');
        if (!root) {
            root = document.createElement('div');
            root.id = 'ec_export_root';
            document.body.appendChild(root);
        }
        return root;
    }

    function getExportCardBackground() {
        const content = document.getElementById('discordContent');
        const bar = document.getElementById('discordBar');
        const bg = content ? window.getComputedStyle(content).backgroundColor : '';
        const barBg = bar ? window.getComputedStyle(bar).backgroundColor : '';
        if (bg && bg !== 'transparent' && !bg.startsWith('rgba(0, 0, 0, 0)')) return bg;
        if (barBg && barBg !== 'transparent' && !barBg.startsWith('rgba')) return barBg;
        return '#111214';
    }

    function getLiveCommentaryExportNode() {
        const content = document.getElementById('discordContent');
        if (!content) return null;
        const container = content.querySelector('.discord_container');
        if (container) return container.cloneNode(true);
        return content.cloneNode(true);
    }

    function getExportReferenceWidth() {
        const content = document.getElementById('discordContent');
        if (!content) return 600;
        const container = content.querySelector('.discord_container') || content;
        const rect = container.getBoundingClientRect();
        const width = Math.ceil(rect.width || content.clientWidth || 0);
        return Math.max(280, width || 600);
    }

    function sanitizeExportClone(clone) {
        if (!clone) return clone;
        clone.classList.add('ec_export_content_clone');
        clone.querySelectorAll('.ec_status_overlay, .discord_status, .ec_typing_indicator, .ec_post_indicator').forEach(el => el.remove());
        return clone;
    }

    function buildCommentsOnlyExportNode() {
        const clone = sanitizeExportClone(getLiveCommentaryExportNode());
        if (!clone) return null;
        const wrapper = document.createElement('div');
        wrapper.className = 'ec_export_comments_only';
        wrapper.setAttribute('data-style', String(settings.style || 'realdiscord'));
        wrapper.style.width = `${getExportReferenceWidth()}px`;
        wrapper.style.background = getExportCardBackground();
        wrapper.appendChild(clone);
        return wrapper;
    }

    function normalizeExportUsernames(exportRoot) {
        const usernameEls = exportRoot.querySelectorAll('.rd_username');
        usernameEls.forEach(el => {
            const inlineStyle = el.getAttribute('style') || '';
            const computed = window.getComputedStyle(el);
            const bgImage = `${computed.backgroundImage || ''} ${inlineStyle}`;
            const textFill = computed.getPropertyValue('-webkit-text-fill-color') || '';
            const hasGradient = /linear-gradient\(/i.test(bgImage);
            const hasTransparentFill = /transparent|rgba\(0,\s*0,\s*0,\s*0\)/i.test(textFill);
            if (!hasGradient && !hasTransparentFill) return;

            let fallbackColor = computed.color || '';
            if (!fallbackColor || /rgba\(0,\s*0,\s*0,\s*0\)|transparent/i.test(fallbackColor)) {
                const match = bgImage.match(/#(?:[0-9a-f]{3,8})\b|rgba?\([^\)]+\)/i);
                fallbackColor = match ? match[0] : '#9aa9ff';
            }
    ...
    async function ensureHtml2CanvasLoaded() {
        ...
        await new Promise((resolve, reject) => {
            const existing = document.querySelector('script[data-echolite-html2canvas]');
            if (existing) {
            ...
            s.dataset.echoliteHtml2canvas = '1';
            s.onload = resolve;
    ...
    async function exportCommentaryAsImage(format = 'png') {
        const content = document.getElementById('discordContent');
        ...
        try {
            ...
            const exportCard = buildCommentsOnlyExportNode();
            if (!exportCard) throw new Error('Export clone build failed');

            exportRoot.appendChild(exportCard);
            normalizeExportUsernames(exportRoot);

            const measuredWidth = Math.ceil(exportCard.scrollWidth);
            const measuredHeight = Math.ceil(exportCard.scrollHeight);

            const canvas = await window.html2canvas(exportCard, {
                backgroundColor: getExportCardBackground(),
                scale: 2,
                useCORS: true,
                logging: false,
                allowTaint: true,
                ...
                windowHeight: measuredHeight + 64,
                scrollX: 0,
                scrollY: 0,
            });
            ...
            const fileName = `echolite_post${meta.postId}_swipe${meta.swipeIdx}_${meta.timestamp}.${ext}`;

            canvas.toBlob(blob => {
                if (!blob) {
                    toastr.error(t('EchoLite Generation Error') + ': canvas empty');
                    return;
                }
                triggerBlobDownload(blob, fileName);
                toastr.success(ext === 'png' ? 'Export PNG' : t('Export JPG'));
            }, mime, quality);
        } catch (e) {
    ...
    function findScrollContainer(el) {
        ...
        while (node && node !== document.body && node !== document.documentElement) {
            ...
            const isScrollable = /(auto|scroll|overlay)/.test(overflowY) && node.scrollHeight > node.clientHeight + 4;
    ...
    function initPostScrollObserver() {
        ...
        if (!chatEl) return;
        const scrollEl = findScrollContainer(chatEl);
        ...
        function handleBestVisible(bestEl) {
            ...
            // Always try to restore cache for the currently viewed post first,
            // including hidden posts that were previously generated while visible.
            restoreCachedCommentary(String(msgId), swipeIdx);
            setCurrentPost(String(msgId), swipeIdx);

            // Hidden posts are display-only: cache can be shown, but generation must not run.
            if (!isCommentaryGenerationEligible(msg)) return;
        ...
        if (scrollEl === window) {
            ...
            window.addEventListener('touchend', debouncedScrollCheck, { passive: true });
        } else {
            scrollEl.addEventListener('scroll', debouncedScrollCheck, { passive: true });
            scrollEl.addEventListener('touchend', debouncedScrollCheck, { passive: true });
        }
        _postScrollEl   = scrollEl;
        ...
        function observeAIEl(el) {
            ...
            if (!msgid) return;
            const msg = getSTContext().chat?.[parseInt(msgid)];
            if (!isCommentaryDisplayEligible(msg)) return;
        ...
        _postMutObserver.observe(chatEl, { childList: true, subtree: true });
    }
    ...
    function buildPostNavOptions() {
        try {
            ...
            for (let i = 0; i < chat.length; i++) {
                const msg = chat[i];
                if (!isCommentaryDisplayEligible(msg)) continue;
                const swipeIdx    = typeof msg.swipe_id === 'number' ? msg.swipe_id : 0;
                const totalSwipes = msg.swipes?.length || 1;
                ...
                const hiddenMark  = msg.is_hidden ? ' [hidden]' : '';
                const isSelected  = String(i) === String(currentPostId);
                options.push(
                    `<option value="${i}" data-swipe="${swipeIdx}"${isSelected ? ' selected' : ''}>${pinIcon}#${i + 1} [${swipeIdx + 1}/${totalSwipes}]${hiddenMark} ${escapeHtml(preview)}…</option>`
                );
            }
    ...
    function toggleQSMenu(btn) {
            ...
            ${!settings.noSaveMode ? `<div class="ec_of_accordion ec_of_acc_open">
                ...
                <div class="ec_of_acc_body ec_of_open" id="ec_qs_nav_body">
                    ...
                    <div class="ec_of_chip_wrap" style="margin-top:6px">
                        <div class="ec_of_chip" id="ec_qs_nav_export" style="flex:1;justify-content:center">${t('Export Image')}</div>
                    </div>
                </div>
            </div>` : `<div class="ec_of_accordion ec_of_acc_open">
                <div class="ec_of_acc_body ec_of_open" id="ec_qs_nav_body" style="padding:4px 8px">
                    <div class="ec_of_chip_wrap"><div class="ec_of_chip" id="ec_qs_nav_export" style="flex:1;justify-content:center">${t('Export Image')}</div></div>
                </div>
            </div>`}
        ...
        menu.querySelector('#ec_qs_nav_select')?.addEventListener('change', function () {
            ...
            // Hidden posts are display-only: show cached commentary if available,
            // but never generate new commentary from prompt-excluded content.
            if (!isCommentaryGenerationEligible(msg)) {
                restoreCachedCommentary(msgId, swipeIdx);
                setCurrentPost(msgId, swipeIdx);
                return;
            }
        ...
        // Post Navigator: Export Image button
        ...
        menu.querySelectorAll('[data-qs-pos]').forEach(el => {
            el.addEventListener('click', function () {
                settings.position = getEffectivePanelPosition(this.dataset.qsPos);
                saveSettings();
                closeQSMenu();
    ...
    function toggleOverflowMenu(btn) {
        ...
        menu.querySelectorAll('[data-pos]').forEach(el => {
            el.addEventListener('click', function () {
                settings.position = getEffectivePanelPosition(this.dataset.pos);
                saveSettings();
                closeOverflowMenu();
                removePanel(); renderPanel();
            });
...
<div id="ec_settings_modal">
    ...
    <div class="ecm_card">
        <div class="ecm_header">
            <div class="ecm_header_title"><i class="fa-solid fa-gear"></i> ${t('EchoLite Settings')}</div>
        ...
        modal.on('change', '[name="ecm_pos"]', function () { settings.position = getEffectivePanelPosition(this.value); saveSettings(); removePanel(); renderPanel(); });
        ...
        modal.on('change', '#ecm_max_tokens', function () {
            const parsedValue = Number.parseInt(this.value, 10);
            settings.maxTokens = Number.isFinite(parsedValue) && parsedValue > 0 ? parsedValue : '';
            if (settings.maxTokens === '') this.value = '';
            saveSettings();
        ...
        modal.on('change', '#ecm_openai_url', function () {
            settings.openaiUrl = normalizeOpenAIBaseUrl(this.value);
            this.value = settings.openaiUrl;
            saveSettings();
        });
            ...
            if (ctx.eventTypes.MESSAGE_UPDATED) {
                ctx.eventSource.on(ctx.eventTypes.MESSAGE_UPDATED, (msgId) => {
                    ...
                    if (msg.is_hidden) {
                        ...
                        // The post was just returned into prompt visibility.
                        // ST may recreate the DOM node without producing a fresh scroll/IO event,
                        // so cached commentary must be restorable immediately, not only after
                        // the next manual scroll.
                        const msgIdStr = String(msgId);
                        const swipeIdx = getMessageSwipeIdx(msgIdStr);

                        // If this exact post already has cached commentary, prefer restoring it
                        // as soon as it becomes visible to EchoLite again.
                        // Safe cases:
                        // 1) it was the currently selected post before hide/unhide,
                        // 2) the bar has no current post selected,
                        // 3) the bar content is empty (nothing meaningful to preserve).
                        const hasCached = !!getCachedPost(msgIdStr, swipeIdx);
                        const barText = extractText(document.getElementById('discordContent')?.innerHTML || '').trim();
                        if (hasCached && (
                            String(msgIdStr) === String(currentPostId) ||
                            currentPostId === null ||
                            !barText
                        )) {
                            restoreCachedCommentary(msgIdStr, swipeIdx);
                        }
                    }
